...
namespace libdas {
    ...
    uint64_t id_generator::randUI64() {
        uint64_t out = 0;
            ...
            if (!CryptAcquireContext(&crypt_prov, NULL, NULL, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT | CRYPT_SILENT)) {
                fprintf(stderr, "Failed to acquire WIN32 cryptographic context\n");
                exit(-1);
            }
            if(!CryptGenRandom(crypt_prov, sizeof(uint64_t), (BYTE*) &out)) {
                fprintf(stderr, "Failed to generate random cryptographically secure number\n");
                exit(-1);
            }
        #endif
    ...
    uint32_t id_generator::randUI32() {
        uint64_t tmp = randUI64();
        return *(uint32_t*) &tmp;
    }


    uint16_t id_generator::randUI16() {
        uint64_t tmp = randUI64();
        return *(uint16_t*) &tmp;
    }


    uint8_t id_generator::randUI8() {
        uint64_t tmp = randUI64();
        return *(uint8_t*) &tmp;
    }
    ...
    const char *id_generator::getDeviceMacAddress() {
        ...
        size_t offset = 0;
            ...
            if((ret_val = GetAdaptersInfo(p_ad_info, &out_buf_len)) != ERROR_SUCCESS) {
                fprintf(stderr, "Failed to retrieve information about network devices\n");
                exit(-1);
            }
    ...
    /// Generate new uuid for usage
    void id_generator::newID() {
        const auto tp = std::chrono::system_clock::now();

        // Pseudo randomise the time value
        uint32_t tval[2];
        tval[0] = (uint32_t) std::chrono::duration_cast<std::chrono::seconds>(tp.time_since_epoch()).count() ^ randUI32();
        tval[1] = (uint32_t) std::chrono::duration_cast<std::chrono::seconds>(tp.time_since_epoch()).count() ^ randUI32();
        ...
        const char *mac = getDeviceMacAddress();
        uint32_t mac_hex = strtol(mac, NULL, 16);
        mac_hex ^= mac_randomizer;

        sprintf(m_id.bytes, "%08x-%04x-%04x-%04x-%08x", *(uint32_t*) (__uuid.bytes), *(uint16_t*) (__uuid.bytes + 4), 
                *(uint16_t*) (__uuid.bytes + 6), *(uint16_t*) (__uuid.bytes + 8), mac_hex);
    }


    /// Get the current generated UUID
    uuid_t &id_generator::getUUID() {
        return m_id;
    }
}
