...
namespace Libdas {
    ...
    class LIBDAS_API WavefrontObjCompiler : private DasWriterCore {
        private:
            struct Vertex {
                TRS::Point3D<float> pos = { FLT_MAX, FLT_MAX, FLT_MAX };
                TRS::Point2D<float> uv = { FLT_MAX, FLT_MAX };
                TRS::Point3D<float> normal = { FLT_MAX, FLT_MAX, FLT_MAX };
            ...
            std::vector<TRS::Point3D<float>> m_unique_pos;
            std::vector<TRS::Point2D<float>> m_unique_uv;
