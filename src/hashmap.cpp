...
/// file: hashmap.h - simple C hashmap source file
...
extern "C" {
    ...
    void newHashmap (
        ...
        p_hashmap->map_data = (__HashData*) calloc(p_hashmap->map_size, sizeof(__HashData));
        p_hashmap->indices = (int64_t*) calloc(p_hashmap->map_size, sizeof(size_t));
    }
     ...
     * 5. Return hash % n
     */
    static size_t __hashfunc(void *key, size_t key_size, size_t map_c) {
        size_t i;
    ...
    static void __reallocateHashmap(Hashmap *p_hm) {
        ...
        for(i = 0; i < old_size; i++) {
            if(p_hm->map_data[i].key) {
                ...
                if((tmp[j].data || tmp[j].key) && p_hm->map_data[i].key_len < tmp[j].key_len ? 
                   ...
                   memcmp(p_hm->map_data[i].key, tmp[j].key, tmp[j].key_len)) {
                    ...
                    for(i = (int64_t) j; i < (int64_t) p_hm->map_size; i++) {
    ...
    void **getHashmapList(Hashmap *p_hm) {
        void **out = (void**) calloc(p_hm->used_size, sizeof(void*));
        for(uint64_t i = 0; i < p_hm->used_size; i++)
