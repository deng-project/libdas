...
namespace Libdas {
    ...
    class LIBDAS_API GLTFCompiler : private DasWriterCore {
        ...
        private:
            ...
            Libdas::Vector2<float> _GetUV(BufferAccessorData &_ad, uint32_t _index);
            Libdas::Vector4<float> _GetColorMultiplier(BufferAccessorData &_ad, uint32_t _index);
            Libdas::Vector4<uint16_t> _GetJointIndices(BufferAccessorData &_ad, uint32_t _index);
            Libdas::Vector4<float> _GetJointWeights(BufferAccessorData &_ad, uint32_t _index);
